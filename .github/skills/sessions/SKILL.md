...
- **Hiding the aux-bar composite bar globally**: Continue chat CSS must only hide the aux-bar title/composite bar when Continue Chat (`#workbench.panel.chat`) owns that panel. A global `display: none` on `.composite-bar-container` removes the Agents Window **Files / Changes** switch tabs. Scope with `:has(.composite#workbench\.panel\.chat)`, and keep a `.agent-sessions-workbench` force-show safeguard if needed.
