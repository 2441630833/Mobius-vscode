...
export class ConversationFeature implements IExtensionContribution {
	...
	constructor(
		...
		// Mobius / Continue-first: users often never sign into GitHub Copilot. Do not leave
		// extension activate() hanging on Copilot token — that prevents languageModelTool
		// implementations (copilot_multiReplaceString, …) from finishing registration, while
		// package.json tool *data* is already visible → "does not have an implementation registered".
		if (!activationBlockerDeferred.isSettled) {
			this.logService.info('ConversationFeature: unblocking activation so IDE languageModelTools can register (auth optional)');
			activationBlockerDeferred.complete();
		}
	}
