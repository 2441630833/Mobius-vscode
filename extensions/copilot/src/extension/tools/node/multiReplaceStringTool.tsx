...
export class MultiReplaceStringTool extends AbstractReplaceStringTool<IMultiReplaceStringToolParams> {
	...
	async invoke(options: vscode.LanguageModelToolInvocationOptions<IMultiReplaceStringToolParams>, token: vscode.CancellationToken) {
		const normalized = normalizeMultiReplaceInput(options.input);
		if (!normalized.replacements.length) {
			throw new Error('Invalid input, no replacements array');
		}
		options = { ...options, input: normalized };
...
function normalizeMultiReplaceInput(input: IMultiReplaceStringToolParams | Record<string, unknown> | undefined): IMultiReplaceStringToolParams {
	const raw = (input ?? {}) as Record<string, unknown>;
	const edits = Array.isArray(raw.replacements) ? raw.replacements
		: Array.isArray(raw.edits) ? raw.edits
			: Array.isArray(raw.changes) ? raw.changes
				: [];
	const topPath = typeof raw.filePath === 'string' ? raw.filePath
	...
	const replacements = edits
		.map((e) => {
			...
			const filePath = String(row.filePath ?? row.filepath ?? row.path ?? row.file_path ?? topPath ?? '');
			const oldString = String(row.oldString ?? row.old_string ?? row.old_str ?? row.find ?? '');
			const newString = String(row.newString ?? row.new_string ?? row.new_str ?? row.replace ?? '');
			if (!filePath.trim() || oldString.length === 0) {
				return undefined;
			}
			return { filePath, oldString, newString };
		})
		.filter((e): e is IAbstractReplaceStringInput => !!e);

	return {
		explanation: typeof raw.explanation === 'string' ? raw.explanation : 'Apply multiple edits',
		replacements,
	};
}
