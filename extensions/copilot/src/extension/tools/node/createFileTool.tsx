...
import { dirname, extname } from '../../../util/vs/base/common/resources';
...
import { ensureExternalInvokePromptContext, finalizeExternalInvokeStream, isExternalInvokePromptContext } from './externalInvokePromptContext';
...
export class CreateFileTool implements ICopilotTool<ICreateFileParams> {
	...
	async invoke(options: vscode.LanguageModelToolInvocationOptions<ICreateFileParams>, token: vscode.CancellationToken) {
		// Continue Agent / direct lm.invokeTool never call resolveInput — synthesize a stream that applies WorkspaceEdit.
		this._promptContext = ensureExternalInvokePromptContext(this._promptContext, options.chatRequestId);
		...
		const externalInvoke = isExternalInvokePromptContext(this._promptContext);
		...
		if (hasSupportedNotebooks) {
			...
			// Continue Agent / direct lm.invokeTool: WorkspaceEdit textEdit on brand-new files
			// often fails outside Copilot's chat edit session. Write the bytes directly.
			if (externalInvoke) {
			...
			this.sendTelemetry(options.chatRequestId, modelId, fileExtension);
			await finalizeExternalInvokeStream(this._promptContext);
			return new LanguageModelToolResult([
