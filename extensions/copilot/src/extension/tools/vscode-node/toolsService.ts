...
export class ToolsService extends BaseToolsService {
	...
	constructor(
		...
		this._copilotTools = new Lazy(() => {
			const map = new Map<ToolName, ICopilotTool<unknown>>();
			for (const ctor of ToolRegistry.getTools()) {
				try {
					map.set(ctor.toolName, _instantiationService.createInstance(ctor));
				} catch (err) {
					logService.error(`Failed to instantiate Copilot tool ${ctor.toolName}`, err);
				}
			}
			return map;
		});
		this._toolExtensions = new Lazy(() => {
			const map = new Map<ToolName, ICopilotToolExtension<unknown>>();
			for (const ctor of ToolRegistry.getToolExtensions()) {
				try {
					map.set(ctor.toolName, _instantiationService.createInstance(ctor));
				} catch (err) {
					logService.error(`Failed to instantiate Copilot tool extension ${ctor.toolName}`, err);
				}
			}
			return map;
		});
