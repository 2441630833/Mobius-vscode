...
import './allTools';
import { ENSURE_COPILOT_TOOLS_MOUNTED_COMMAND, mountCopilotLanguageModelTools } from './mountLanguageModelTools';
...
export class ToolsContribution extends Disposable {
	constructor(
		...
		// Mount ALL Copilot languageModelTool implementations immediately on activation.
		mountCopilotLanguageModelTools(toolsService, this.logService, {
			push: (d) => this._register(d as vscode.Disposable),
		});

		this._register(vscode.commands.registerCommand(ENSURE_COPILOT_TOOLS_MOUNTED_COMMAND, () => {
			return mountCopilotLanguageModelTools(toolsService, this.logService, {
				push: (d) => this._register(d as vscode.Disposable),
			});
		}));
