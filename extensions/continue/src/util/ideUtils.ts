...
export class VsCodeIdeUtils {
  ...
  async readFile(
    ...
    // First check if there's an open UTF-8 document with this URI that might
    // have unsaved changes. Non-UTF-8 open docs (e.g. gbk) must NOT be used —
    // their getText() turns on-disk UTF-8 Chinese into mojibake.
    const openDocuments = vscode.workspace.textDocuments;
    for (const document of openDocuments) {
      if (document.uri.toString() === uri.toString()) {
        const enc = (document.encoding ?? "utf8")
          .toLowerCase()
          .replace(/[^a-z0-9]/g, "");
        const isUtf8 =
          enc === "utf8" || enc === "utf8bom" || enc === "utf8withbom";
        if (isUtf8 || document.isDirty) {
          const docText = document.getText();
          return Buffer.from(docText, "utf8");
        }
        break; // clean non-UTF-8 tab → fall through to disk UTF-8 bytes
      }
    }

    // If no suitable open document, fall back to reading from disk.
