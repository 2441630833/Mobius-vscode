...
import { CONTINUE_CHAT_VIEW_ID } from "../util/chatViewId";
...
export class VsCodeExtension {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    // Mobius: Continue renders in the core chat auxiliary bar (Copilot slot).
    ...
    void (async () => {
      ...
      ensureBundledLocalModels(workspaceRoot, vscode.env.appRoot);

      // Defer reload so the first GUI config load is not competing with reloadConfig.
      await new Promise((resolve) => setTimeout(resolve, 5000));
      await this.configHandler.reloadConfig("Synced Mobius model settings");
    ...
    void (async () => {
      ...
      fs.watchFile(workspaceEnvPath, { interval: 1000 }, async (stats) => {
        if (stats.size === 0) {
          return;
        }
        // Sync Continue config from workspace/global .env only — never rewrite
        // workspace .env (that would undo user git reverts).
        syncModelEnvFromFiles(workspaceRoot);
        await this.configHandler.reloadConfig(
          "Workspace .env updated - fs file watch",
        );
        void vscode.commands.executeCommand(
          "workbench.action.continue.reloadLanguageModels",
        );
