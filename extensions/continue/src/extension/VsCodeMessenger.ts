...
export class VsCodeMessenger {
  ...
  constructor(
    ...
    this.onWebview("toggleMaximizedAuxiliaryBar", () => {
      void vscode.commands.executeCommand(
        "workbench.action.toggleMaximizedAuxiliaryBar",
      );
    });

    this.onWebview("closeAuxiliaryBar", () => {
    ...
    this.onWebview("physicalAI/saveModelEnv", async ({ data }) => {
      const dirs = await this.ide.getWorkspaceDirs();
      const workspaceRoot = dirs.length ? vscode.Uri.parse(dirs[0]).fsPath : "";
      ...
      if (!apiKey && !isOllamaProvider(provider)) {
        return {
          ok: false,
      ...
      try {
        const configHandler = await configHandlerPromise;
        const profileId =
          configHandler.currentProfile?.profileDescription.id ?? "local";
        ...
        const saveResult = saveModelEnv(
          workspaceRoot,
          { ...settings, profileId: envProfileId },
          profileId,
        );
        await configHandler.reloadConfig("Mobius model settings saved");
        void vscode.commands.executeCommand(
          "workbench.action.continue.reloadLanguageModels",
        );
        const notice = saveResult.warning ?? warning;
        if (notice) {
    ...
    this.onWebview("physicalAI/resetModelEnv", async () => {
      const dirs = await this.ide.getWorkspaceDirs();
      const workspaceRoot = dirs.length ? vscode.Uri.parse(dirs[0]).fsPath : "";
      ...
      try {
        ...
        const profileId =
          configHandler.currentProfile?.profileDescription.id ?? "local";
        const { settings, warning } = resetModelEnvToDefault(
          workspaceRoot,
          profileId,
        );
        await configHandler.reloadConfig("Mobius model settings reset");
        void vscode.commands.executeCommand(
          "workbench.action.continue.reloadLanguageModels",
        );
        if (warning) {
    ...
    this.onWebview("physicalAI/getOpenAiEnv", async () => {
      const dirs = await this.ide.getWorkspaceDirs();
      if (!dirs.length) {
        return null;
      }
      const workspaceRoot = vscode.Uri.parse(dirs[0]).fsPath;
      const settings = loadModelEnv(workspaceRoot);
      return {
    ...
    this.onWebview("physicalAI/saveOpenAiEnv", async ({ data }) => {
      const dirs = await this.ide.getWorkspaceDirs();
      if (!dirs.length) {
      ...
      try {
        ...
        const profileId =
          configHandler.currentProfile?.profileDescription.id ?? "local";
        const current = loadModelEnv(workspaceRoot);
        ...
        saveModelEnv(workspaceRoot, settings, profileId);
        await configHandler.reloadConfig("Mobius model settings saved");
        void vscode.commands.executeCommand(
          "workbench.action.continue.reloadLanguageModels",
        );
