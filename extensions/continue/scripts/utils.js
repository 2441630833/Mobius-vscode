...
async function downloadRipgrepBinary(target) {
  ...
  const assetName = assetByTarget[target];
  if (!assetName) {
    throw new Error(`Unsupported ripgrep target: ${target}`);
  }

  const directUrl = `https://github.com/microsoft/ripgrep-prebuilt/releases/download/v13.0.0-10/${assetName}`;
  const downloadUrls = [
    `https://gh-proxy.org/${directUrl}`,
    `https://v4.gh-proxy.org/${directUrl}`,
    `https://v6.gh-proxy.org/${directUrl}`,
    directUrl,
  ];

  if (target.startsWith("win")) {
    const zipPath = path.join("node_modules/@vscode/ripgrep/bin/build.zip");
    let downloaded = false;
    for (const downloadUrl of downloadUrls) {
      try {
        console.log(`Trying download: ${downloadUrl}`);
        execCmdSync(`curl -L -o "${zipPath}" "${downloadUrl}"`);
    ...
    if (!downloaded) {
      throw new Error("Could not download ripgrep binary");
    }

    const extractDir = path.join("node_modules/@vscode/ripgrep/bin/extract");
    ...
    const walk = (dir) => {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          walk(fullPath);
        } else if (entry.name === "rg.exe") {
          rgCandidates.push(fullPath);
        }
      }
    };
    walk(extractDir);
    if (rgCandidates.length === 0) {
      throw new Error("rg.exe not found in downloaded ripgrep archive");
    }
    fs.copyFileSync(
      rgCandidates[0],
      path.join("node_modules/@vscode/ripgrep/bin/rg.exe"),
    );
    rimrafSync(extractDir);
    return;
  }
  ...
  for (const downloadUrl of downloadUrls) {
    try {
      ...
      if (
        fs.existsSync("node_modules/@vscode/ripgrep/bin/build.tar.gz") &&
        fs.statSync("node_modules/@vscode/ripgrep/bin/build.tar.gz").size > 1000
      ) {
        downloaded = true;
        break;
      }
