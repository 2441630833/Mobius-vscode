...
import { ChatViewContainerId } from '../../../contrib/chat/browser/chat.js';
...
/** Mutable for Mobius Chat single-header mode (PartLayout reads hasTitle each layout). */
type MutablePartOptions = { hasTitle?: boolean };
...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	constructor(
		...
		// Mobius: Chat uses an in-webview header; reclaim the 35px title band for the overlay webview.
		this._register(this.onDidPaneCompositeOpen(() => this.updateMobiusChatHeaderMode()));
		this._register(this.onDidPaneCompositeClose(() => this.updateMobiusChatHeaderMode()));
		queueMicrotask(() => this.updateMobiusChatHeaderMode());
	}

	/**
	 * When Chat is active, drop title layout height so Continue's webview fills the part.
	 * Title actions are rendered inside the webview TabBar / Layout instead.
	 */
	private updateMobiusChatHeaderMode(): void {
		const isChat = this.getActivePaneComposite()?.getId() === ChatViewContainerId;
		const wantTitle = !isChat;
		const options = this.options as MutablePartOptions;
		if (options.hasTitle !== wantTitle) {
			options.hasTitle = wantTitle;
		}
		if (this.titleArea) {
			this.titleArea.style.display = wantTitle ? '' : 'none';
		}
