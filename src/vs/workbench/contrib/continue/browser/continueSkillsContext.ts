...
/**
 * Extra fused score for Mobius self-evolving skills (`.agents/skills/auto/`).
 * Generic Claude/GitHub skills get INTENT_HINTS (+6) and domain deltas; without
 * this prior, generated skills almost never occupy a top-3 Auto-routed slot.
 */
const MOBIUS_AUTO_SKILL_FUSED_BOOST = 14;
/** Extra fused score for other workspace/personal `.agents/skills/` (not auto/). */
const MOBIUS_AGENTS_SKILL_FUSED_BOOST = 6;
/** Lexical prior so auto-generated skills enter LEXICAL_RECALL_TOP_K. */
const MOBIUS_AUTO_SKILL_LEXICAL_PRIOR = 3;
const MOBIUS_AGENTS_SKILL_LEXICAL_PRIOR = 1;
...
export function buildContinueSkillsContextFast(
	...
	hybridDetails = applyMobiusGeneratedSkillBoost(hybridDetails);
	...
	const ordered = selectAutoRoutedSkills(
...
export function rankSkillsForMessage(message: string, skills: readonly IAgentSkill[]): RankedSkill[] {
	...
	const ranked: RankedSkill[] = skills.map(skill => {
		...
		// Description lexical overlap (CJK 2-grams are length 2; Latin noise is skipped below 4)
		let descHits = 0;
		for (const t of descTokens) {
			const isCjk = /[\u4e00-\u9fff]/.test(t);
			if (isCjk ? t.length < 2 : t.length < 4) {
				continue;
			}
		...
		if (isMobiusAutoGeneratedSkill(skill.uri)) {
			score += MOBIUS_AUTO_SKILL_LEXICAL_PRIOR;
		} else if (isMobiusAgentsFolderSkill(skill.uri)) {
			score += MOBIUS_AGENTS_SKILL_LEXICAL_PRIOR;
		}
...
/**
 * Prefer Mobius-authored skills under `.agents/skills` over generic local
 * Claude/GitHub/Copilot skills. Boost is gated on some lexical or embedding
 * overlap so unrelated generated skills do not always occupy a slot.
 */
export function applyMobiusGeneratedSkillBoost<T extends {
	skill: IAgentSkill;
	fusedScore: number;
	lexicalScore?: number;
	embedScore?: number;
}>(details: readonly T[]): T[] {
	return details.map(d => {
		if (!isMobiusAgentsFolderSkill(d.skill.uri)) {
			return d;
		}
		const lexical = d.lexicalScore ?? 0;
		const embed = d.embedScore ?? 0;
		// Lexical prior is only for recall ranking — do not treat it as overlap.
		const overlap = lexical - mobiusGeneratedSkillLexicalPrior(d.skill.uri);
		if (overlap < 1 && embed < 2) {
			return d;
		}
		if (isMobiusAutoGeneratedSkill(d.skill.uri)) {
			return { ...d, fusedScore: d.fusedScore + MOBIUS_AUTO_SKILL_FUSED_BOOST };
		}
		return { ...d, fusedScore: d.fusedScore + MOBIUS_AGENTS_SKILL_FUSED_BOOST };
	});
}

function mobiusGeneratedSkillLexicalPrior(uri: URI): number {
	if (isMobiusAutoGeneratedSkill(uri)) {
		return MOBIUS_AUTO_SKILL_LEXICAL_PRIOR;
	}
	if (isMobiusAgentsFolderSkill(uri)) {
		return MOBIUS_AGENTS_SKILL_LEXICAL_PRIOR;
	}
	return 0;
}

/** Self-evolving skills written by ContinueSelfEvolving. */
export function isMobiusAutoGeneratedSkill(uri: URI): boolean {
	return normalizeSkillPath(uri).includes('/.agents/skills/auto/');
}

/** Workspace or personal `.agents/skills/` (includes auto/). */
export function isMobiusAgentsFolderSkill(uri: URI): boolean {
	return normalizeSkillPath(uri).includes('/.agents/skills/');
}

function normalizeSkillPath(uri: URI): string {
...
/**
 * Auto-route selection: keep conflict-aware greedy fill, but always try to
 * include the best Mobius `.agents` skill (prefer `skills/auto/`) in the top N.
 */
export function selectAutoRoutedSkills(
	candidates: readonly SkillLoadCandidate[],
	max: number,
	message?: string,
): SkillLoadCandidate[] {
	...
	const preferred = candidates.find(c => isMobiusAutoGeneratedSkill(c.uri) && c.score >= AUTO_ROUTE_SCORE_THRESHOLD)
		?? candidates.find(c => isMobiusAgentsFolderSkill(c.uri) && c.score >= AUTO_ROUTE_SCORE_THRESHOLD);
	if (!preferred) {
		return selectCompatibleSkills(candidates, max, message);
	}
	const others = candidates.filter(c => c.uri.toString() !== preferred.uri.toString());
	const rest = selectCompatibleSkills(others, Math.max(0, max - 1), message)
		.filter(c => !skillsConflict(preferred.name, c.name, message));
...
export function simulateLexicalSkillRouting(
	...
	details = applyMobiusGeneratedSkillBoost(details);
	...
	const routed = selectAutoRoutedSkills(candidates, max, message).map(c => c.name);
	return {
		routed,
...
function buildSkillsCatalog(skills: readonly IAgentSkill[]): string {
	...
	let charCount = 0;
	const catalogSkills = [...skills].sort((a, b) =>
		mobiusSkillCatalogRank(b) - mobiusSkillCatalogRank(a) || a.name.localeCompare(b.name));
	let truncatedAt = catalogSkills.length;
	for (let i = 0; i < catalogSkills.length; i++) {
		const skill = catalogSkills[i];
		const skillEntry = [
			'<skill>',
	...
	if (truncatedAt < catalogSkills.length) {
		const names = catalogSkills.slice(truncatedAt).map(s => s.name);
...
function mobiusSkillCatalogRank(skill: IAgentSkill): number {
	if (isMobiusAutoGeneratedSkill(skill.uri)) {
		return 2;
	}
	if (isMobiusAgentsFolderSkill(skill.uri)) {
		return 1;
	}
	return 0;
}
