...
import { registerContinueChatFileDropTarget } from '../../continue/browser/continueChatDragDrop.js';
...
export class WebviewViewPane extends ViewPane {
	...
	protected override renderBody(container: HTMLElement): void {
		...
		// Register Continue file drop target on the pane body (this is called
		// after `this.element` exists so hit-testing via getBoundingClientRect
		// works correctly).
		this._register(registerContinueChatFileDropTarget(container, this.id, this.commandService, this.fileService));
	...
	private activate() {
		...
		// NOTE: `dragenter`/`dragover`/`drop` are intentionally NOT re-dispatched here —
		// the Continue chat drop handler (registered on this.element / container) needs
		// to see explorer file drags at the window capture phase before the webview
		// iframe's overlay interferes.
		for (const event of [EventType.DRAG, EventType.DRAG_END, EventType.DRAG_LEAVE, EventType.DRAG_START]) {
