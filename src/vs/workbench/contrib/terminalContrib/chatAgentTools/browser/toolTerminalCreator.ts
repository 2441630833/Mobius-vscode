...
export class ToolTerminalCreator {
	...
	private _createCopilotTerminal(shellOrProfile: string | ITerminalProfile, os: OperatingSystem) {
		...
		const config: IShellLaunchConfig = {
			...
			// Mobius: show agent/tool terminals in the Terminal tab list so users can
			// select and close long-running servers (port conflicts) instead of
			// collapsing them into "N Hidden Terminals".
			hideFromUser: false,
			forcePersist: true,
			env,
		};
