...
import { ChatModel, type IChatRequestModeInfo, type IChatResponseModel } from '../../../../chat/common/model/chatModel.js';
...
/**
 * True when assistant text asserts the user task is finished.
 * Kept in sync with Continue's `assertsTaskComplete` heuristics so late
 * background-terminal steering is suppressed even when the model omits the
 * literal `TASK_COMPLETE` token (common Chinese/English "done" closers).
 */
export function textIndicatesTaskComplete(text: string): boolean {
	if (!text.trim()) {
		return false;
	}
...
/**
 * Mobius / Continue agents end a finished turn with `TASK_COMPLETE` (or an
 * equivalent done closer). Late background-terminal completion steering must
 * not start a new chat request after that marker.
 *
 * Prefer markdown / final-response text over `toString()`: the full repr can
 * drop earlier content after `clearToPreviousToolInvocation`, which would
 * hide `TASK_COMPLETE` if we only inspected `toString()`.
 */
export function responseIndicatesTaskComplete(response: IChatResponseModel | undefined): boolean {
	if (!response || response.isCanceled) {
		return false;
	}
	try {
		const entire = response.entireResponse;
		const visible = response.response;
		const text = [
			entire?.getMarkdown?.() ?? '',
			entire?.getFinalResponse?.() ?? '',
			entire?.toString?.() ?? '',
			visible?.getMarkdown?.() ?? '',
			visible?.getFinalResponse?.() ?? '',
			visible?.toString?.() ?? '',
		].join('\n');
		return textIndicatesTaskComplete(text);
	} catch {
		return false;
	}
}
...
/**
 * True when the chat session still has an active agent turn that can receive
 * background-terminal steering (Pending / NeedsInput). When the session is idle
 * (no response, complete, or canceled), completed/exited/input-needed steering
 * must not open a new System Notification turn.
 */
export function sessionHasInProgressResponse(lastRequest: { response?: IChatResponseModel } | undefined): boolean {
	const response = lastRequest?.response;
	if (!response) {
		return false;
	}
	return !response.isComplete && !response.isCanceled;
}
...
export class RunInTerminalTool extends Disposable implements IToolImpl {
	...
	/**
	 * Maps background-notification terminal instance IDs to the chat session that
	 * registered them, so Continue can dispose pending steering when a turn ends
	 * with TASK_COMPLETE (even if response-text detection races the pty exit).
	 */
	private readonly _backgroundNotificationSessions = new Map<number, string>();

	/** Singleton pointer for static session-level suppress helpers. */
	private static _instance: RunInTerminalTool | undefined;
	...
	/**
	 * Dispose all pending background-terminal completion listeners for a chat
	 * session. Called when Continue finishes a turn with TASK_COMPLETE so a late
	 * `terminal exited` / `command completed` event cannot open a new agent turn.
	 */
	public static suppressBackgroundSteeringForSession(sessionResource: URI): void {
		RunInTerminalTool._instance?._disposeBackgroundNotificationsForSession(sessionResource);
	}
	...
	private _disposeBackgroundNotificationsForSession(sessionResource: URI): void {
		const sessionKey = sessionResource.toString();
		for (const [instanceId, session] of [...this._backgroundNotificationSessions]) {
			if (session === sessionKey) {
				this._logService.debug(`RunInTerminalTool: Disposing background steering for terminal instance ${instanceId} after session task complete`);
				this._backgroundNotifications.deleteAndDispose(instanceId);
				this._backgroundNotificationSessions.delete(instanceId);
			}
		}
	}
	...
	constructor(
		...
		super();
		RunInTerminalTool._instance = this;
		this._register(toDisposable(() => {
			if (RunInTerminalTool._instance === this) {
				RunInTerminalTool._instance = undefined;
			}
		}));
	...
	async prepareToolInvocation(context: IToolInvocationPreparationContext, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
		...
		const isSessionAutoApproved = chatSessionResource && isSessionAutoApproveLevel(chatSessionResource, this._configurationService, this._chatWidgetService, this._chatService, context.chatRequestId);
		...
		if (!isBackground) {
			...
			if (cachedTerminal && !cachedTerminal.isBackground && !cachedTerminal.instance.isDisposed) {
				...
				if (cachedTerminal.instance.exitCode !== undefined) {
					...
					this._backgroundNotificationSessions.delete(cachedTerminal.instance.instanceId);
	...
	private _registerCompletionNotification(terminalInstance: ITerminalInstance, termId: string, chatSessionResource: URI, commandName: string, toolSpecificData: IChatTerminalToolInvocationData, outputMonitor?: OutputMonitor, alreadyNotifiedInputNeededOutput?: string): void {
		...
		this._backgroundNotificationSessions.delete(notificationKey);
		...
		// The request that started this background command. Prefer it over
		// lastRequest when deciding suppression so a later user turn cannot
		// accidentally "unsilence" a TASK_COMPLETE finish from the launcher.
		const launchingRequest = lastRequest;

		// If the user manually stopped the agent, the session is idle (no
		// in-progress response), or the agent closed the turn with TASK_COMPLETE,
		// suppress background steering and tear down the notification listeners.
		//
		// Without the idle hard gate, a late onCommandFinished (common on
		// Windows for long-running scripts that were backgrounded by timeout /
		// async mode) calls sendRequest with systemInitiatedLabel "{cmd} completed"
		// after the agent already finished its turn — even when TASK_COMPLETE was
		// never emitted. That renders in the user-request lane and starts a new
		// unwanted agent turn.
		const handleSessionShouldSuppressSteering = (): boolean => {
			if (sessionRef.object.lastRequest?.response?.isCanceled) {
				disposeNotification();
				return true;
			}
			if (!sessionHasInProgressResponse(sessionRef.object.lastRequest)) {
				this._logService.debug(`RunInTerminalTool: Suppressing steering for terminal ${termId} because session is idle`);
				disposeNotification();
				return true;
			}
			if (
				responseIndicatesTaskComplete(launchingRequest?.response)
				|| responseIndicatesTaskComplete(sessionRef.object.lastRequest?.response)
			) {
				this._logService.debug(`RunInTerminalTool: Suppressing steering for terminal ${termId} because agent emitted TASK_COMPLETE`);
				disposeNotification();
				return true;
			}
			return false;
		...
		const disposeIfRequestClosedForSteering = (response: IChatResponseModel | undefined): void => {
			if (!response) {
				return;
			}
			if (response.isCanceled || response.isComplete || responseIndicatesTaskComplete(response)) {
				disposeNotification();
			}
		};

		// Proactively tear down listeners when the launching / current request
		// is canceled or finishes with TASK_COMPLETE, rather than waiting for
		// the next terminal event.
		if (launchingRequest?.response) {
			store.add(launchingRequest.response.onDidChange(ev => {
				if (ev.reason === 'completedRequest') {
					disposeIfRequestClosedForSteering(launchingRequest.response);
				}
			}));
		}
		store.add(autorun(reader => {
			...
			reader.store.add(request.response.onDidChange(ev => {
				if (ev.reason === 'completedRequest') {
					disposeIfRequestClosedForSteering(request.response);
				}
			}));
		}));
		...
		if (outputMonitor) {
			...
			store.add(outputMonitor.onDidDetectInputNeeded(() => {
				...
				if (handleSessionShouldSuppressSteering()) {
					return;
				}
		...
		this._backgroundNotificationSessions.set(notificationKey, chatSessionResource.toString());
