...
suite('RunInTerminalTool', () => {
	...
	/** Stub acquireExistingSession with an in-progress (or complete) response for steering tests. */
	function stubSessionLastRequest(options: {
		modelId?: string;
		agentId?: string;
		isComplete?: boolean;
		isCanceled?: boolean;
		entireResponse?: { toString?: () => string; getMarkdown?: () => string; getFinalResponse?: () => string };
	} = {}) {
		const previousRequest = {
			modelId: options.modelId ?? 'test-model',
			response: {
				agent: { id: options.agentId ?? 'continue' },
				isCanceled: options.isCanceled ?? false,
				isComplete: options.isComplete ?? false,
				entireResponse: options.entireResponse,
				onDidChange: Event.None,
			},
		};
	...
	test('should use the conversation model and preserve previous agent for background completion notifications', async () => {
		...
		stubSessionLastRequest({ modelId: previousModelId, agentId: previousAgentId, isComplete: false });
	...
	test('should suppress background completion steering when session is idle', async () => {
		const termId = 'test-completion-idle-term';
		const sessionResource = LocalChatSessionUri.forSession('test-completion-idle-session');
		const commandFinishedEmitter = new Emitter<{ exitCode: number | undefined }>();
		const terminalDisposedEmitter = new Emitter<void>();
		const inputDataEmitter = new Emitter<string>();
		...
		// Complete turn without TASK_COMPLETE — the idle hard gate must still suppress.
		stubSessionLastRequest({
			isComplete: true,
			entireResponse: {
				toString: () => 'Started the server in the background.',
				getMarkdown: () => 'Started the server in the background.',
				getFinalResponse: () => 'Started the server in the background.',
			},
		});
		...
		const toolSpecificData = { kind: 'terminal', commandLine: { original: '$env:WECOM_ALLOW_ALL_USERS = "true"; npm run dev' }, language: 'pwsh' } as IChatTerminalToolInvocationData;
		// eslint-disable-next-line @typescript-eslint/naming-convention
		(runInTerminalTool as unknown as { _registerCompletionNotification: (terminal: ITerminalInstance, termId: string, session: URI, commandName: string, toolSpecificData: IChatTerminalToolInvocationData) => void })
			._registerCompletionNotification(terminalInstance, termId, sessionResource, '$env:WECOM_ALLOW_ALL_USERS = "true"; npm run dev', toolSpecificData);
		await new Promise<void>(resolve => setTimeout(resolve, 0));

		commandFinishedEmitter.fire({ exitCode: 0 });

		strictEqual(capturedSteeringRequests.length, 0, 'Idle session must not send completed System Notification steering');
	});

	test('should suppress terminal-exited steering when session is idle', async () => {
		const termId = 'test-exited-idle-term';
		const sessionResource = LocalChatSessionUri.forSession('test-exited-idle-session');
		const commandFinishedEmitter = new Emitter<{ exitCode: number | undefined }>();
		const terminalDisposedEmitter = new Emitter<void>();
		const inputDataEmitter = new Emitter<string>();
		...
		stubSessionLastRequest({
			isComplete: true,
			entireResponse: {
				toString: () => 'Background process left running.',
				getMarkdown: () => 'Background process left running.',
			},
		});
		...
		const toolSpecificData = { kind: 'terminal', commandLine: { original: 'npm run dev' }, language: 'pwsh' } as IChatTerminalToolInvocationData;
		// eslint-disable-next-line @typescript-eslint/naming-convention
		(runInTerminalTool as unknown as { _registerCompletionNotification: (terminal: ITerminalInstance, termId: string, session: URI, commandName: string, toolSpecificData: IChatTerminalToolInvocationData) => void })
			._registerCompletionNotification(terminalInstance, termId, sessionResource, 'npm run dev', toolSpecificData);
		await new Promise<void>(resolve => setTimeout(resolve, 0));

		terminalDisposedEmitter.fire();

		strictEqual(capturedSteeringRequests.length, 0, 'Idle session must not send terminal-exited System Notification steering');
	});

	test('should suppress background completion steering after TASK_COMPLETE', async () => {
		const termId = 'test-completion-task-complete-term';
		const sessionResource = LocalChatSessionUri.forSession('test-completion-task-complete-session');
		const commandFinishedEmitter = new Emitter<{ exitCode: number | undefined }>();
		const terminalDisposedEmitter = new Emitter<void>();
		const inputDataEmitter = new Emitter<string>();
		...
		const previousRequest = {
			...
			response: {
				...
				isComplete: true,
				entireResponse: {
					// Prefer getMarkdown: toString may omit TASK_COMPLETE after clearToPreviousToolInvocation.
					toString: () => 'tool output only',
					getMarkdown: () => 'Done.\nTASK_COMPLETE\n',
					getFinalResponse: () => 'Done.\nTASK_COMPLETE\n',
				},
				onDidChange: Event.None,
			},
		};
		...
		(runInTerminalTool.constructor as unknown as { _activeExecutions: Map<string, { getOutput(): string; dispose(): void; instance: ITerminalInstance }> })._activeExecutions.set(termId, {
			getOutput: () => 'server started',
			dispose: () => { },
			instance: terminalInstance,
		});

		const toolSpecificData = { kind: 'terminal', commandLine: { original: 'cd d:\\AI\\Doc_Agent; .\\scripts\\start.ps1' }, language: 'pwsh' } as IChatTerminalToolInvocationData;
		// eslint-disable-next-line @typescript-eslint/naming-convention
		(runInTerminalTool as unknown as { _registerCompletionNotification: (terminal: ITerminalInstance, termId: string, session: URI, commandName: string, toolSpecificData: IChatTerminalToolInvocationData) => void })
			._registerCompletionNotification(terminalInstance, termId, sessionResource, 'cd d:\\AI\\Doc_Agent; .\\scripts\\start.ps1', toolSpecificData);
		await new Promise<void>(resolve => setTimeout(resolve, 0));

		commandFinishedEmitter.fire({ exitCode: 0 });

		strictEqual(capturedSteeringRequests.length, 0, 'TASK_COMPLETE must suppress late background completion steering');
	});

	test('should suppress terminal-exited steering after TASK_COMPLETE', async () => {
		const termId = 'test-exited-task-complete-term';
		const sessionResource = LocalChatSessionUri.forSession('test-exited-task-complete-session');
		const commandFinishedEmitter = new Emitter<{ exitCode: number | undefined }>();
		const terminalDisposedEmitter = new Emitter<void>();
		const inputDataEmitter = new Emitter<string>();
		...
		const toolSpecificData = { kind: 'terminal', commandLine: { original: 'curl -s http://127.0.0.1:8765/api/dns-lookup?host=yunwu.ai' }, language: 'pwsh' } as IChatTerminalToolInvocationData;
		// eslint-disable-next-line @typescript-eslint/naming-convention
		(runInTerminalTool as unknown as { _registerCompletionNotification: (terminal: ITerminalInstance, termId: string, session: URI, commandName: string, toolSpecificData: IChatTerminalToolInvocationData) => void })
			._registerCompletionNotification(terminalInstance, termId, sessionResource, 'curl -s http://127.0.0.1:8765/api/dns-lookup?host=yunwu.ai', toolSpecificData);
		await new Promise<void>(resolve => setTimeout(resolve, 0));

		terminalDisposedEmitter.fire();

		strictEqual(capturedSteeringRequests.length, 0, 'TASK_COMPLETE-equivalent finish must suppress terminal-exited System Notification');
	});

	test('should suppress terminal-exited steering via suppressBackgroundSteeringForSession', async () => {
		const termId = 'test-exited-suppress-session-term';
		const sessionResource = LocalChatSessionUri.forSession('test-exited-suppress-session');
		const commandFinishedEmitter = new Emitter<{ exitCode: number | undefined }>();
		const terminalDisposedEmitter = new Emitter<void>();
		const inputDataEmitter = new Emitter<string>();
		...
		const previousRequest = {
			...
			response: {
				...
				isComplete: true,
				entireResponse: {
					toString: () => 'still working, no done marker',
					getMarkdown: () => 'still working, no done marker',
					getFinalResponse: () => 'still working, no done marker',
				},
				onDidChange: Event.None,
			},
		};
		...
		const toolSpecificData = { kind: 'terminal', commandLine: { original: 'Get-Process node' }, language: 'pwsh' } as IChatTerminalToolInvocationData;
		// eslint-disable-next-line @typescript-eslint/naming-convention
		(runInTerminalTool as unknown as { _registerCompletionNotification: (terminal: ITerminalInstance, termId: string, session: URI, commandName: string, toolSpecificData: IChatTerminalToolInvocationData) => void })
			._registerCompletionNotification(terminalInstance, termId, sessionResource, 'Get-Process node', toolSpecificData);
		await new Promise<void>(resolve => setTimeout(resolve, 0));

		RunInTerminalTool.suppressBackgroundSteeringForSession(sessionResource);
		terminalDisposedEmitter.fire();

		strictEqual(capturedSteeringRequests.length, 0, 'Continue TASK_COMPLETE path must dispose pending terminal-exited steering');
	});
	...
	test('should dedupe rapid repeated background input-needed notifications', () => {
		...
		stubSessionLastRequest({ isComplete: false });
