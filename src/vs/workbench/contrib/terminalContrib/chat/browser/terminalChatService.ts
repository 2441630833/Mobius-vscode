...
export class TerminalChatService extends Disposable implements ITerminalChatService {
	...
	getToolSessionTerminalInstances(hiddenOnly?: boolean): readonly ITerminalInstance[] {
		if (hiddenOnly) {
			...
			// Only count terminals still created as hideFromUser. Mobius agent shells
			// are visible tabs; when another Agents session backgrounds them they must
			// not resurface as "N Hidden Terminals".
			return Array.from(uniqueInstances).filter(i =>
				!!i.shellLaunchConfig.hideFromUser && !foregroundInstances.has(i.instanceId));
		}
