...
registerAction2(class ShowChatTerminalsAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// Use hiddenOnly so visible Mobius agent tabs (hideFromUser: false) that are
		// merely backgrounded for another Agents session are not listed here.
		const hiddenToolInstances = terminalChatService.getToolSessionTerminalInstances(true);

		const all = new Map<number, ITerminalInstance>();
		for (const i of hiddenToolInstances) {
