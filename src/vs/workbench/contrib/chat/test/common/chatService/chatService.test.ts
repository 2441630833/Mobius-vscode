...
suite('ChatService', () => {
	...
	test('system-initiated request injects agent-side progress label', async () => {
		const testService = createChatService();
		const chatModelRef = testDisposables.add(startSessionModel(testService));
		const chatModel = chatModelRef.object;

		const response = await testService.sendRequest(chatModel.sessionResource, '[Terminal notification: command completed]\nTerminal output:\nok', {
			isSystemInitiated: true,
			systemInitiatedLabel: '`npm test` completed',
		});
		ChatSendResult.assertSent(response);
		await response.data.responseCompletePromise;

		const request = chatModel.getRequests()[0];
		assert.strictEqual(request.isSystemInitiated, true);
		assert.strictEqual(request.systemInitiatedLabel, '`npm test` completed');
		assert.ok(request.response);
		assert.ok(
			request.response.progressMessages.some(p => p.content.value === '`npm test` completed'),
			'systemInitiatedLabel must appear as an agent-side progressMessage',
		);
	});
