...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatModeKind, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { agent: SetupAgent; disposable: IDisposable } {
		return instantiationService.invokeFunction(accessor => {
			...
			const agentDisplayName = isContinuePhysicalAiIde()
				? product.nameShort
				: `${defaultChat.provider.default.name} Copilot`; /* Do NOT change for Copilot — hides the username altogether in Chat */
			return SetupAgent.doRegisterAgent(instantiationService, chatAgentService, id, agentDisplayName, true, description, location, mode, context, controller);
		});
	}
	...
	static registerBuiltInAgents(instantiationService: IInstantiationService, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): IDisposable {
		return instantiationService.invokeFunction(accessor => {
			...
			const { disposable: vscodeDisposable } = SetupAgent.doRegisterAgent(instantiationService, chatAgentService, 'setup.vscode', 'vscode', false, localize2('vscodeAgentDescription', "Ask questions about Mobius").value, ChatAgentLocation.Chat, ChatModeKind.Agent, context, controller);
			...
			disposables.add(SetupTool.registerTool(instantiationService, {
				...
				modelDescription: 'Scaffold a new workspace in Mobius',
				userDescription: localize('setupToolsDescription', "Scaffold a new workspace in Mobius"),
