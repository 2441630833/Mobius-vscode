...
import { isMobiusAgentSessionAutoApproved } from '../../../continue/browser/continueAgentAutoApprove.js';
...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	constructor(
		...
		this.vscodeToolSet = this._register(this.createToolSet(
			...
			VSCodeToolReference.vscode,
			{
				...
				description: localize('copilot.toolSet.vscode.description', 'Use Mobius features'),
			}
		));
	...
	hasToolImplementation(id: string): boolean {
		return !!this._tools.get(id)?.impl;
	}
	 ...
	 */
	private _isSessionInAutoApproveLevel(chatSessionResource: URI | undefined, chatRequestId?: string): boolean {
		if (!chatSessionResource) {
			return false;
		}
		if (isMobiusAgentSessionAutoApproved(chatSessionResource, this._chatWidgetService, this._chatService, chatRequestId)) {
			return true;
		}
	...
	private async shouldAutoConfirm(toolId: string, runsInWorkspace: boolean | undefined, source: ToolDataSource, parameters: unknown, chatSessionResource: URI | undefined, chatRequestId: string | undefined, combination?: { label: string; key: string }, workingDirectory?: URI): Promise<ConfirmedReason | undefined> {
		...
		if (chatSessionResource && this._isSessionInAutoApproveLevel(chatSessionResource, chatRequestId)) {
	...
	private async shouldAutoConfirmPostExecution(toolId: string, runsInWorkspace: boolean | undefined, source: ToolDataSource, parameters: unknown, chatSessionResource: URI | undefined, chatRequestId: string | undefined, workingDirectory?: URI): Promise<ConfirmedReason | undefined> {
		...
		const sessionAutoApprove = chatSessionResource && this._isSessionInAutoApproveLevel(chatSessionResource, chatRequestId);
