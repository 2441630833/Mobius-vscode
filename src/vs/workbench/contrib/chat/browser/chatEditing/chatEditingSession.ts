...
export class ChatEditingSession extends Disposable implements IChatEditingSession {
	...
	/** External file edits tracker: maps each modified file URI to its content BEFORE the external edit was applied. */
	// private readonly _externalEditBeforeContents = new ResourceMap<string>();
	/** Tracks which response/request IDs have had their initial file contents captured. */
	// private readonly _capturedInitialContentForRequestId = new Set<string>();
	...
	/**
	 * Returns the checkpoint ID for a request/undoStop pair, or undefined if
	 * no such checkpoint exists. Delegates to the underlying timeline.
	 */
	public getCheckpointIdForRequest(requestId: string, undoStopId?: string): string | undefined {
		return this._timeline.getCheckpointIdForRequest(requestId, undoStopId);
	}
	...
	public async restoreSnapshot(requestId: string, stopId: string | undefined): Promise<void> {
		...
		// "Restore Checkpoint" should undo the request's edits by navigating
		// to the state BEFORE the request, not TO the request's own checkpoint
		// (which represents the state AFTER edits). This matches
		// AgentHostSnapshotController which targets index - 1.
		if (stopId === undefined) {
			const timelineState = this._timeline.getStateForPersistence();
			const checkpoints = timelineState.checkpoints;
			const targetIdx = checkpoints.findIndex(c => c.checkpointId === checkpointId);
			if (targetIdx > 0) {
				await this._timeline.navigateToCheckpoint(checkpoints[targetIdx - 1].checkpointId);
			}
			return;
		}
	...
	private async _createModifiedFileEntry(resource: URI, telemetryInfo: IModifiedEntryTelemetryInfo, ifNotExists: NotExistBehavior, initialContent: string | undefined): Promise<AbstractChatEditingModifiedFileEntry | undefined> {
		...
		try {
			...
			if (this._configurationService.getValue<boolean>('accessibility.openChatEditedFiles')) {
