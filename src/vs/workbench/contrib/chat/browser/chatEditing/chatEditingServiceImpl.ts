...
export class ChatEditingService extends Disposable implements IChatEditingService {
		...
		const observerDisposables = new DisposableStore();
				...
				observerDisposables.add(chatModel.onDidChange(async e => {
					if (e.kind === 'addRequest') {
						...
						// When the response is created after the request (streaming path),
						// set up the edit observer so that textEditGroup, notebookEditGroup,
						// workspaceEdit, and externalEdit parts create checkpoints in the
						// editing session. Without this, "Restore Checkpoint" silently does
						// nothing for responses that stream after the addRequest event.
						const requestId = e.response.requestId;
						this.observerEditsInResponse(requestId, e.response, session, observerDisposables);
					}
				}));
	...
	observerEditsInResponse(requestId: string, responseModel: IChatResponseModel, session: ChatEditingSession, observerDisposables: DisposableStore) {
			...
			// Track whether the response contains external edits so we can
			// ensure a checkpoint exists when the response completes.
			let hasExternalEdits = false;
			...
			const onResponseComplete = () => {
				...
				// If the response had external edits but no streaming edits were
				// set up (e.g. Continue agent writes files directly), ensure a
				// checkpoint exists so "Restore Checkpoint" has a boundary to
				// navigate to. The checkpoint was already seeded by
				// session.createSnapshot() when the request was added, but if
				// the timeline's current epoch moved forward since then (e.g.
				// intermediate undo stops were created) the restore may target
				// a stale epoch. We re-create the checkpoint at the current
				// epoch so the timeline has a fresh terminal boundary.
				if (hasExternalEdits && !session.getCheckpointIdForRequest(requestId, undefined)) {
					session.createSnapshot(requestId, undefined);
				}
			};
