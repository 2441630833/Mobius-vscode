...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	private _createModelPickerDelegate(): IModelPickerDelegate {
		return {
			...
			showAutoModel: () => isContinuePhysicalAiIde() ? false : this._showAutoModel(),
	...
	*/
	...
	private _setEmptyModelState() {
		...
		if (isContinuePhysicalAiIde() && this.currentModeKind === ChatModeKind.Agent) {
			this.setPermissionLevel(ChatPermissionLevel.AutoApprove);
		} else if (currentLevel === undefined || !isChatPermissionLevel(currentLevel)) {
			this.setPermissionLevel(this.getDefaultPermissionLevel());
		}
	...
	private _syncFromModel(state: IChatModelInputState | undefined, forSessionResource: URI): void {
		...
		try {
			...
			if (!this.configurationService.getValue<boolean>(ChatConfiguration.GlobalAutoApprove)) {
				let targetLevel = this.getPermittedPermissionLevel(state?.permissionLevel ?? ChatPermissionLevel.Default);
				const mobiusAgentOverride = isContinuePhysicalAiIde() && this.currentModeKind === ChatModeKind.Agent;
				if (mobiusAgentOverride) {
					targetLevel = ChatPermissionLevel.AutoApprove;
				}
				...
				if (mobiusAgentOverride && state?.permissionLevel !== ChatPermissionLevel.AutoApprove) {
					queueMicrotask(() => this.setPermissionLevel(ChatPermissionLevel.AutoApprove));
				}
			}
