...
function getNoModelsPickerLabel(): string {
	// Mobius: "No models available" reads as a product error when the user
	// simply hasn't opened a folder yet / models are still registering.
	if (isContinuePhysicalAiIde()) {
		return localize('chat.modelPicker.loadingModels', "Loading models…");
	}
	return localize('chat.modelPicker.noModels', "No models available");
}
...
function getUpdateHoverContent(updateState: StateType): MarkdownString {
	...
	switch (updateState) {
		case StateType.AvailableForDownload:
			hoverContent.appendMarkdown(localize('chat.modelPicker.downloadUpdateHover', "This model requires a newer version of {0}. [Download Update](command:update.downloadUpdate) to access it.", product.nameShort));
			break;
		case StateType.Downloaded:
		case StateType.Ready:
			hoverContent.appendMarkdown(localize('chat.modelPicker.restartUpdateHover', "This model requires a newer version of {0}. [Restart to Update](command:update.restartToUpdate) to access it.", product.nameShort));
			break;
		default:
			hoverContent.appendMarkdown(localize('chat.modelPicker.checkUpdateHover', "This model requires a newer version of {0}. [Update {0}](command:update.checkForUpdate) to access it.", product.nameShort));
			break;
	}
	return hoverContent;
}
...
/** Mobius Agents: edit an existing .env provider profile (QuickInput). */
function createMobiusManageProvidersAction(commandService: ICommandService): IActionWidgetDropdownAction {
	return {
		id: 'mobiusManageProviders',
		enabled: true,
		checked: false,
		class: ThemeIcon.asClassName(Codicon.gear),
		tooltip: localize('chat.mobius.manageProviders.tooltip', "Modify AI providers"),
		label: localize('chat.mobius.manageProviders', "Modify Providers..."),
		run: async () => {
			try {
				await commandService.executeCommand('continue.openModelProviderSettings');
			} catch {
				// Extension may not be ready — try workbench alias.
				await commandService.executeCommand('workbench.action.continue.openModelProviderSettings');
			}
		}
	};
}

/** Mobius Agents: add a new .env provider profile (QuickInput). */
function createMobiusAddProviderAction(commandService: ICommandService): IActionWidgetDropdownAction {
	return {
		id: 'mobiusAddProvider',
		enabled: true,
		checked: false,
		class: ThemeIcon.asClassName(Codicon.add),
		tooltip: localize('chat.mobius.addProvider.tooltip', "Add a new AI provider profile"),
		label: localize('chat.mobius.addProvider', "Add Provider..."),
		run: async () => {
			try {
				await commandService.executeCommand('continue.addModelProvider');
			} catch {
				await commandService.executeCommand('workbench.action.continue.addModelProvider');
			}
		}
	};
}

/** Mobius Agents: delete an existing .env provider profile (QuickInput). */
function createMobiusDeleteProviderAction(commandService: ICommandService): IActionWidgetDropdownAction {
	return {
		id: 'mobiusDeleteProvider',
		enabled: true,
		checked: false,
		class: ThemeIcon.asClassName(Codicon.trash),
		tooltip: localize('chat.mobius.deleteProvider.tooltip', "Delete an AI provider profile"),
		label: localize('chat.mobius.deleteProvider', "Delete Provider..."),
		run: async () => {
			try {
				await commandService.executeCommand('continue.deleteModelProvider');
			} catch {
				await commandService.executeCommand('workbench.action.continue.deleteModelProvider');
			}
		}
	};
}
		...
		if (!showAutoModel) {
			...
			items.push({
				item: {
					...
					checked: false,
					class: undefined,
					tooltip: getNoModelsPickerLabel(),
					label: getNoModelsPickerLabel(),
					run: () => { }
				},
				kind: ActionListItemKind.Action,
				label: getNoModelsPickerLabel(),
...
function createUnavailableModelItem(
	...
	if (reason === 'upgrade') {
		...
		description = localize('chat.modelPicker.updateDescription', "Update {0}", product.nameShort);
	} else {
...
export class ModelPickerWidget extends Disposable {
	...
	show(anchor?: HTMLElement): void {
		...
		const isMobius = isContinuePhysicalAiIde();
		...
		const mobiusAddProviderAction = isMobius ? createMobiusAddProviderAction(this._commandService) : undefined;
				const mobiusDeleteProviderAction = isMobius ? createMobiusDeleteProviderAction(this._commandService) : undefined;
				const mobiusManageProvidersAction = isMobius ? createMobiusManageProvidersAction(this._commandService) : undefined;
		...
		// Mobius: every Continue/.env profile defaults to pinned in the Agents picker.
		let pinnedIds = this._languageModelsService.getPinnedModelIds().filter(id => !this._languageModelsService.isModelHidden(id));
		if (isMobius) {
			const continueIds = models
				.filter(m => m.metadata.vendor === 'continue')
				.map(m => m.identifier);
			for (const id of continueIds) {
				this._languageModelsService.pinModel(id);
			}
		...
		// Mobius: Add / Modify provider entries at the bottom of the picker list
		// (visible in both empty-state "new session" and active chat input pickers).
		if (isMobius && (mobiusAddProviderAction || mobiusDeleteProviderAction || mobiusManageProvidersAction)) {
					items.push({ kind: ActionListItemKind.Separator, label: localize('chat.mobius.providersSection', "Providers") });
					if (mobiusAddProviderAction) {
						items.push(createModelItem(mobiusAddProviderAction));
					}
					if (mobiusDeleteProviderAction) {
						items.push(createModelItem(mobiusDeleteProviderAction));
					}
					if (mobiusManageProvidersAction) {
						items.push(createModelItem(mobiusManageProvidersAction));
					}
				}
		...
		const filterActions = isMobius
					? [mobiusAddProviderAction, mobiusDeleteProviderAction, mobiusManageProvidersAction].filter((a): a is IActionWidgetDropdownAction => !!a)
		...
		const listOptions = {
			...
			filterActions: filterActions?.length ? filterActions : undefined,
		...
		const delegate = {
			onSelect: (action: IActionWidgetDropdownAction) => {
				this._actionWidgetService.hide();
				// Await async actions (Mobius Add/Modify Provider QuickInput).
