...
export class ChatThinkingContentPart extends ChatCollapsibleContentPart implements IChatContentPart {
	...
	constructor(
		...
		this._register(autorun(r => {
			...
			// Expanding while streaming: stick to the latest thinking text (fixedScrolling
			// previously skipped scroll updates when not collapsed).
			if (isExpanded && this.fixedScrollingMode && !this.streamingCompleted) {
	...
	protected override initContent(): HTMLElement {
		...
		if (this.fixedScrollingMode) {
			...
			// so we can update scroll dimensions when content grows while streaming.
			...
			// Once content exceeds max-height (collapsed), the wrapper box size stops changing
	...
	private updateScrollDimensionsFromCache(): void {
		...
		// Collapsed: fixed 200px viewport that sticky-scrolls.
		// Expanded: grow with full content so the chat list can follow the latest thinking.
		const viewportHeight = isCollapsed
			? Math.min(contentHeight, THINKING_SCROLL_MAX_HEIGHT)
			: contentHeight;
		...
		try {
			...
			if (this.autoScrollEnabled) {
				this.scrollToBottom(contentHeight, viewportHeight);
			}
		} finally {
			this.isUpdatingDimensions = false;
		}

		this.updateFadeClasses(this.lastKnownScrollTop, this.lastKnownContentHeight, viewportHeight);
		this.updateDropdownClickability(contentHeight);
	}

	private scrollToBottom(contentHeight: number, viewportHeight?: number): void {
		if (!this.scrollableElement) {
			return;
		}

		const vh = viewportHeight ?? Math.min(contentHeight, THINKING_SCROLL_MAX_HEIGHT);

		if (contentHeight > vh) {
			const newScrollTop = contentHeight - vh;
	...
	public updateThinking(content: IChatThinkingPart): void {
		...
		// Growing thinking text changes the response row height — notify so the
		// chat list can stick to the bottom while the user is following along.
		this._onDidChangeHeight.fire();
