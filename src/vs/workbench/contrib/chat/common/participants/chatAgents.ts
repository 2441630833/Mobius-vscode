...
export class ChatAgentService extends Disposable implements IChatAgentService {
	...
	private _preferExtensionAgent<T extends IChatAgentData>(agents: T[]): T | undefined {
		...
		// Prefer the product defaultChatAgent extension when set (Mobius → Continue),
		// otherwise the last extension agent, falling back to the last core agent.
		const preferredExtId = product.defaultChatAgent?.extensionId;
		if (preferredExtId) {
			const preferred = findLast(agents, agent =>
				!agent.isCore && ExtensionIdentifier.equals(agent.extensionId, preferredExtId));
			if (preferred) {
				return preferred;
			}
		}
