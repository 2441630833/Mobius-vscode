			...
			if (providedSession.onDidStartServerRequest) {
				disposables.add(providedSession.onDidStartServerRequest(({ prompt, variableData, isSystemInitiated, systemInitiatedLabel }) => {
					...
					if (isSystemInitiated) {
						const label = (systemInitiatedLabel ?? parsedRequest.text)?.trim();
						if (label) {
							model.acceptResponseProgress(lastRequest, {
		...
		const sendRequestInternal = async () => {
			...
			try {
				...
				if (agentPart || (defaultAgent && !commandPart)) {
					...
					// Render system-initiated terminal/steering labels as an agent-side
					// progress step. The request row is hidden in ChatViewModel so it does
					// not appear as a user-lane "System Notification" bubble.
					if (options?.isSystemInitiated) {
						const label = (options.systemInitiatedLabel ?? parsedRequest.text)?.trim();
						if (label) {
							model.acceptResponseProgress(request, {
								kind: 'progressMessage',
								content: new MarkdownString(label),
