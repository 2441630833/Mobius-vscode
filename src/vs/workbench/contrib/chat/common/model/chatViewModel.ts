...
export class ChatViewModel extends Disposable implements IChatViewModel {
	...
	getItems(): (IChatRequestViewModel | IChatResponseViewModel | IChatPendingDividerViewModel)[] {
		let items: (IChatRequestViewModel | IChatResponseViewModel | IChatPendingDividerViewModel)[] = this._items.filter((item) => {
			...
			// System-initiated terminal/steering requests stay in the model (agent
			// still receives the message) but must not render in the user lane.
			// Their systemInitiatedLabel is shown as a response progressMessage.
			if (item instanceof ChatRequestViewModel && item.isSystemInitiated) {
				return false;
			}
			return true;
		});
		...
		if (pendingRequests.length > 0) {
			// Separate steering and queued requests. Hide system-initiated pending
			// rows (and their "System Notification" divider) so terminal completion
			// never appears as a user-side bubble while waiting to send.
