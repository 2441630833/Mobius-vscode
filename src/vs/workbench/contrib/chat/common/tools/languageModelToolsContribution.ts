		...
		languageModelToolsExtensionPoint.setHandler((_extensions, delta) => {
			for (const extension of delta.added) {
				for (const rawTool of extension.value) {
					...
					// Mobius sets defaultChatAgent.chatExtensionId to Continue.continue, but GitHub.copilot-chat
					// still contributes the IDE languageModelTools — treat it as builtin so tools stay Internal.
					const isBuiltinTool = (() => {
						const chatExtId = productService.defaultChatAgent?.chatExtensionId;
						if (ExtensionIdentifier.equals(extension.description.identifier, 'GitHub.copilot-chat')) {
							return true;
						}
						if (chatExtId) {
							return ExtensionIdentifier.equals(extension.description.identifier, chatExtId);
						}
						return isProposedApiEnabled(extension.description, 'chatParticipantPrivate');
					})();
