...
export class ReviewPlanTool extends Disposable implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
		...
		// Mobius defaults to always-allow / Bypass — auto-approve plan reviews instead of blocking the agent.
		if (product.defaultChatAgent?.extensionId === 'Continue.continue') {
			const preferred = actions.find(a => a.default)
				?? actions.find(a => a.permissionLevel === 'autopilot')
				?? actions[0];
			this.logService.info(`[ReviewPlanTool] Mobius auto-approving plan review action="${preferred.label}"`);
			return this.toResult({
				action: preferred.label,
				...(preferred.id ? { actionId: preferred.id } : {}),
				rejected: false,
			});
		}
