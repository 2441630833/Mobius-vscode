...
/**
 * Maximum recursion depth when traversing subdirectories for skill files.
 * Self-evolving / auto-generated skills are written one level deeper than
 * manual skills (e.g. `.agents/skills/auto/<name>/SKILL.md`, mirroring
 * Hermes Agent's learning loop), so discovery must recurse a few levels or
 * those generated skills would never be found / auto-routed.
 */
const MAX_SKILLS_RECURSION_DEPTH = 3;
...
export class PromptFilesLocator {
	...
	public getAgentFileURIFromModeFile(oldURI: URI): URI | undefined {
		...
		private async findAgentSkillsInFolder(uri: URI, token: CancellationToken, depth: number = 0): Promise<URI[]> {
		try {
			const result: URI[] = [];
			const stat = await this.fileService.resolve(uri);
			if (stat.isDirectory && stat.children) {
				// Recursively traverse subdirectories (up to a depth cap) so
				// that auto-generated skills nested one level deeper
				// (e.g. `.agents/skills/auto/<name>/SKILL.md`) are discovered
				// alongside manual skills (`.agents/skills/<name>/SKILL.md`).
				for (const child of stat.children) {
					try {
						if (token.isCancellationRequested) {
							return [];
						...
						if (child.isDirectory) {
							...
							try {
								...
								// no SKILL.md at this level — keep recursing below
							}
							if (depth < MAX_SKILLS_RECURSION_DEPTH) {
								const nested = await this.findAgentSkillsInFolder(child.resource, token, depth + 1);
								result.push(...nested);
							}
						}
					} catch (error) {
