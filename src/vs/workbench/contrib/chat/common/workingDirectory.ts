...
export class WorkingDirectory {
	 ...
	 *
	 * Absolute paths (Windows drive letter / UNC / POSIX root) and `file:` URIs
	 * are returned as-is — they must not be joined onto the base directory
	 * (that produced doubled paths like `D:\proj\D:\proj\src\a.ts`).
	 *
	...
	resolveRelativePath(filePath: string): URI | undefined {
		...
		// Absolute / URI inputs — do not join onto the working directory.
		...
		const resolved = joinPath(base, trimmed);
