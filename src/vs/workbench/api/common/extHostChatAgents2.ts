...
export class ExtHostChatAgents2 extends Disposable implements ExtHostChatAgentsShape2 {
	...
	private async getModelForRequest(request: IChatAgentRequest, extension: IExtensionDescription): Promise<vscode.LanguageModelChat> {
		...
		if (request.userSelectedModelId) {
			...
			// Mobius: UI may pass Continue model id without vendor prefix.
			if (!model && !request.userSelectedModelId.includes(':')) {
				model = await this._languageModels.getLanguageModelByIdentifier(extension, `continue:${request.userSelectedModelId}`);
			}
		}
		if (!model) {
