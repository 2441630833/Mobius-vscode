...
export class ExtensionEnablementService extends Disposable implements IWorkbenchExtensionEnablementService {
	...
	private ensureChatExtensionInitialDisabledState(): void {
		...
		// Unified chat/completions extension (e.g. Continue.continue in Mobius).
		// Skip Copilot-style onboarding migration that disables the chat extension on fresh profiles.
		if (this._completionsExtensionId && this._completionsExtensionId === this._chatExtensionId) {
			if (this._isDisabledGlobally({ id: this._chatExtensionId })) {
				this.logService.debug('Re-enabling unified chat extension that was disabled by chat enablement migration');
	...
	private _isDisabledByUnification(identifier: IExtensionIdentifier): boolean {
		// Mobius: Continue.continue is both chat and completions — never disable it via unification.
		if (this._completionsExtensionId && this._completionsExtensionId === this._chatExtensionId) {
			return false;
		}
