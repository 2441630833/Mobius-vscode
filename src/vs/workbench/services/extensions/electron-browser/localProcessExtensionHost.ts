...
export class NativeLocalProcessExtensionHost extends Disposable implements IExtensionHost {
	...
	private async _start(): Promise<IMessagePassingProtocol> {
		...
		// Mobius cold start (Continue + bundled extensions) often exceeds VS Code's default
		// window; do not surface a sticky warning on first launch — log only unless the
		// host is paused waiting for a debugger.
		const extensionHostStartupTimeoutMs = 60_000;
		...
		if (!this._environmentService.isBuilt && !this._environmentService.remoteAuthority || this._isExtensionDevHost) {
			startupTimeoutHandle = setTimeout(() => {
				this._logService.error(`[LocalProcessExtensionHost]: Extension host did not start in ${extensionHostStartupTimeoutMs / 1000} seconds (debugBrk: ${this._isExtensionDevDebugBrk})`);
