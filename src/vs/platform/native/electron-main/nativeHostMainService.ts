...
export class NativeHostMainService extends Disposable implements INativeHostMainService {
	...
	async showToast(windowId: number | undefined, options: IToastOptions): Promise<IToastResult> {
		...
		return new Promise<IToastResult>(r => {
			...
			// Focus the window that created the toast when the user activates it.
			// Resolve first so focus-driven clearToasts() cannot race and report clicked:false.
			// This also avoids Windows shell activation opening a second Mobius instance/window.
			const resolveActivated = (result: IToastResult) => {
				resolve(result);
				this.windowById(undefined, windowId)?.focus({ mode: FocusMode.Force });
			};

			disposables.add(cts.token.onCancellationRequested(() => resolve({ supported: true, clicked: false })));

			toast.on('click', () => resolveActivated({ supported: true, clicked: true }));
			toast.on('action', (_event, actionIndex) => resolveActivated({ supported: true, clicked: true, actionIndex }));
