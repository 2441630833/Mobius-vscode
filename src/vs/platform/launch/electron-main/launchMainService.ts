...
import { isMobiusProduct, shouldDefaultToAgentsWindow } from '../../product/common/mobiusAgentsStartup.js';
...
export class LaunchMainService implements ILaunchMainService {
	...
	private async startOpenWindow(args: NativeParsedArgs, userEnv: IProcessEnvironment): Promise<void> {
		...
		// Mobius: Windows toast / Start Menu activation often re-launches with empty args
		// (or without an explicit new-window request). Always prefer focusing the already
		// open window — never open a fresh IDE/Agents window for that activation.
		else if (
			isMobiusProduct(product.defaultChatAgent?.extensionId) &&
			!args['new-window'] &&
			!args['agents'] &&
			!args._.length &&
			!args['folder-uri'] &&
			...
			} else if (shouldDefaultToAgentsWindow(args, product.defaultChatAgent?.extensionId)) {
		...
		// Agents window (explicit --agents, or Mobius default when no paths)
		else if (args['agents'] || shouldDefaultToAgentsWindow(args, product.defaultChatAgent?.extensionId)) {
			usedWindows = await this.windowsMainService.openAgentsWindow(baseConfig);
		}
		...
		// Start without file/folder arguments
			...
			// Otherwise check for settings
				...
				// Mobius: Windows toast/shell activation often re-launches with empty args.
				// Prefer focusing the running window over opening a new empty IDE window
				// (Windows default), while still honoring an explicit user setting of 'on'.
				if (
					openNewWindow &&
					openWithoutArgumentsInNewWindowConfig === 'default' &&
					isMobiusProduct(product.defaultChatAgent?.extensionId) &&
