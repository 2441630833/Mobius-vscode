...
// _VSCODE environment
	...
	if (env['VSCODE_DEV']) {
		Object.assign(product, {
			...
			// Keep Windows toast activation on the Dev process. Sharing the packaged
			// AppUserModelId makes toast clicks launch installed Mobius (new IDE window).
			...(product.win32AppUserModelId ? { win32AppUserModelId: `${product.win32AppUserModelId}.Dev` } : {}),
			...(product.win32MutexName ? { win32MutexName: `${product.win32MutexName}-dev` } : {}),
		});
	}
