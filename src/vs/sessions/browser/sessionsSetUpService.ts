...
function shouldSkipSessionsWelcome(environmentService: IWorkbenchEnvironmentService): boolean {
	// Mobius uses Continue locally — no sign-in gate for the agents window.
	if (product.defaultChatAgent?.extensionId === CONTINUE_EXTENSION_ID) {
		return true;
	}
...
class SessionsSetUpWidget extends Disposable {
	...
	private _completeWithoutSignIn(): void {
