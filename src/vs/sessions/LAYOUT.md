...
**Files / Changes switch:** The aux-bar title composite bar shows **Files** and **Changes** tabs so users can manually switch between the workspace explorer and the session changes list. Do not hide that composite bar in Agents Window (Continue chat CSS must only hide it when the Continue Chat panel owns the classic-IDE aux bar).
