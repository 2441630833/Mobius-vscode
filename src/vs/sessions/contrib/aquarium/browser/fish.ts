...
/**
 * Mobius infinity "fish" used by the Agents window aquarium. Each fish is a
 * small SVG element styled with `color:` so the silhouette inherits via
 * `currentColor`, with animated body strips providing the swimming motion.
 */

/** Brand-tinted variants of the Mobius infinity (pink / lavender / purple). */
...
const SPECIES_COLOR: Record<FishSpecies, string> = {
	[FishSpecies.Stable]: '#E8A8D8',
	[FishSpecies.Insiders]: '#C9A0DC',
	[FishSpecies.Exploration]: '#9B7EBD',
};
 ...
 * a single `<symbol>` containing the Mobius infinity logo path. All fish reference
...
function ensureSharedDefs(targetDocument: Document): void {
	...
	container.appendChild(createMobiusLogoSymbol(targetDocument));
...
function createMobiusLogoSymbol(targetDocument: Document): SVGSymbolElement {
 ...
 *   - Mobius infinity body, sliced into N vertical strips that each oscillate in
...
function buildFishSvg(targetDocument: Document): SVGSVGElement {
	...
	// viewBox 0..96 matches the aquarium strip clip layout.
