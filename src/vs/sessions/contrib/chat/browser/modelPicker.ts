...
import { LOCAL_PROVIDER_ID } from '../../providers/localChatSessions/browser/localChatSessionsProvider.js';
...
/**
 * Provider used for the New Session composer before a folder is picked.
 * Local chat is the default host in Agents / Mobius.
 */
function getDefaultModelProviderId(sessionsProvidersService: ISessionsProvidersService): string {
	return sessionsProvidersService.getProvider(LOCAL_PROVIDER_ID) ? LOCAL_PROVIDER_ID : (sessionsProvidersService.getProviders()[0]?.id ?? LOCAL_PROVIDER_ID);
}

/**
 * Reads the available models for a session from its provider.
 * When no session exists yet (no folder selected), still surface the default
 * provider's models so the picker shows real names instead of "No models available".
 */
function getModelsForSession(session: ISession | undefined, sessionsProvidersService: ISessionsProvidersService): readonly ILanguageModelChatMetadataAndIdentifier[] {
	if (!session) {
		const providerId = getDefaultModelProviderId(sessionsProvidersService);
		return sessionsProvidersService.getProvider(providerId)?.getModels('') ?? [];
	}
...
function getModelPickerOptionsForSession(session: ISession | undefined, sessionsProvidersService: ISessionsProvidersService): ISessionModelPickerOptions {
	const providerId = session?.providerId ?? getDefaultModelProviderId(sessionsProvidersService);
...
export class ModelPicker extends Disposable {
	...
	constructor(
		...
		this._delegate = {
			...
			setModel: (model: ILanguageModelChatMetadataAndIdentifier) => {
				...
				const providerId = session?.providerId ?? getDefaultModelProviderId(this._sessionsProvidersService);
				const sessionType = session?.sessionType ?? 'local';
				this._storageService.store(modelPickerStorageKey(providerId, sessionType), model.identifier, StorageScope.PROFILE, StorageTarget.MACHINE);
				if (session) {
					this._sessionsProvidersService.getProvider(session.providerId)?.setModel(session.sessionId, model.identifier);
					// Sync the selection into the already-loaded ChatModel / widget so that
					// follow-up messages (sent directly via the chat widget, bypassing
					// provider.sendRequest) pick up the new model. Without this, switching
					// models in an existing Agents-window session has no effect until reload.
					this._syncModelToLoadedChat(session, model);
				}
		...
		this._register(autorun(reader => {
			...
			// Keep the model list fresh while this session is active. Before a
			// folder is picked there is no session — still watch the default
			// Local provider so Continue models appear as soon as they register.
			const providerId = session?.providerId ?? getDefaultModelProviderId(this._sessionsProvidersService);
	...
	/**
	 * Push a model selection into the already-loaded ChatModel(s) (and their
	 * widgets) for an existing session. The chat widget sends follow-up messages
	 * directly through {@link IChatService.sendRequest} using its own
	 * `input.currentLanguageModel`, which is derived from the ChatModel's
	 * `inputModel.state.selectedModel` — it does NOT read `session.modelId`.
	 * So we must write the selection into each loaded ChatModel here, otherwise
	 * the picker UI updates but the next sent message still uses the old model.
	 */
	private _syncModelToLoadedChat(session: IActiveSession, model: ILanguageModelChatMetadataAndIdentifier): void {
		// A session may contain multiple chats (primary + sub-chats); sync each
		// one that currently has a live widget or loaded ChatModel.
		const chatResources: URI[] = [];
		const activeChat = session.activeChat.get();
		if (activeChat) {
			chatResources.push(activeChat.resource);
		}
		const mainChat = session.mainChat.get();
		if (mainChat && !chatResources.some(r => r.toString() === mainChat.resource.toString())) {
			chatResources.push(mainChat.resource);
		}
		for (const chat of session.chats.get()) {
			if (!chatResources.some(r => r.toString() === chat.resource.toString())) {
				chatResources.push(chat.resource);
			}
		}

		for (const resource of chatResources) {
			// Prefer the live widget: setCurrentLanguageModel syncs both the widget's
			// observable and (via _syncInputStateToModel) the ChatModel.inputModel.
			const widget = this._chatWidgetService.getWidgetBySessionResource(resource);
			if (widget) {
				widget.input.setCurrentLanguageModel(model);
				continue;
			}
			// No widget mounted yet (e.g. session not focused) — write directly into
			// the ChatModel so the next time a widget binds it picks up the selection.
			const chatModel = this._chatService.getSession(resource);
			if (chatModel) {
				chatModel.inputModel.setState({ selectedModel: model });
			}
		}
	}
	...
	private _getFallbackModel(session: ISession | undefined, models: readonly ILanguageModelChatMetadataAndIdentifier[]): ILanguageModelChatMetadataAndIdentifier {
		const providerId = session?.providerId ?? getDefaultModelProviderId(this._sessionsProvidersService);
		const sessionType = session?.sessionType ?? 'local';
		const rememberedModelId = this._storageService.get(modelPickerStorageKey(providerId, sessionType), StorageScope.PROFILE);
