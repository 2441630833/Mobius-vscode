...
class LocalSession extends Disposable {
	...
	private readonly _permissionLevel = observableValue(
		this,
		isContinuePhysicalAiIde() ? ChatPermissionLevel.AutoApprove : ChatPermissionLevel.Default,
	);
	...
	setMode(mode: IChatMode | undefined): void {
		...
		if (mode) {
			...
			if (isContinuePhysicalAiIde() && mode.kind === ChatModeKind.Agent) {
				this._permissionLevel.set(ChatPermissionLevel.AutoApprove, undefined);
			}
		} else {
...
export class LocalChatSessionsProvider extends Disposable implements ISessionsProvider {
	...
	readonly label = isContinuePhysicalAiIde()
		? localize('localChatSessionsProviderContinue', "Local")
		: localize('localChatSessionsProvider', "Copilot Chat");
	readonly icon = Codicon.vm;
	readonly order = isContinuePhysicalAiIde() ? -1 : 0;
	...
	private _syncSessionFromModel(session: LocalSession): void {
		...
		// Keep ChatModel.workingDirectory aligned with the session folder.
		// Explore tools (ls/grep/codebase) rely on request.workingDirectory; if the
		// model lost it after restore, the agent falls back to the open IDE folder.
		const sessionCwd = session.workspace.get()?.folders[0]?.root;
		if (sessionCwd && model.workingDirectory?.toString() !== sessionCwd.toString()) {
			model.setWorkingDirectory(sessionCwd);
		}
	...
	private _loadPersistedSessions(): void {
		...
		for (const stored of storedSessions) {
			...
			if (isContinuePhysicalAiIde()) {
				session.setPermissionLevel(ChatPermissionLevel.AutoApprove);
			}
			if (stored.archived) {
	...
	getModelPickerOptions(_sessionId: string): ISessionModelPickerOptions {
		// Mobius: Continue cloud chat models only — no Copilot Manage Models / featured list.
		if (isContinuePhysicalAiIde()) {
			return {
	...
	setSessionMode(sessionId: string, mode: IChatMode | undefined): void {
		...
		// Resolve the group primary: the picker is bound to the aggregated group
		// session (primary id), so a model switch must apply to the primary and
		// every draft/idle sub-chat in the group. Without this, switching the
		// model while another chat is running leaves a newly-created sub-chat
		// holding a stale snapshot (taken in _createNewSubsequentChat) and its
		// next message goes out with the old provider. Running chats are left
		// alone so an in-flight request keeps the model it started with.
		const primary = this._resolvePrimary(session);
		for (const chat of this._getGroupChats(primary)) {
			if (chat.status.get() === SessionStatus.InProgress) {
				continue;
			}
	...
	private async _dispatchSend(session: LocalSession, chatResource: URI, options: ISendRequestOptions): ReturnType<IChatService['sendRequest']> {
		...
		const permissionLevel = this._effectivePermissionLevel(session, modeKind);
	...
	/** Mobius Agent sessions bypass tool confirmation prompts (terminal, etc.). */
	private _effectivePermissionLevel(session: LocalSession, modeKind: ChatModeKind): ChatPermissionLevel {
		if (isContinuePhysicalAiIde() && modeKind === ChatModeKind.Agent) {
			return ChatPermissionLevel.AutoApprove;
		}
		return session.permissionLevel.get();
	}
	...
	private _buildGroupISession(primary: LocalSession): ISession {
		...
		const changesets = createChangesets(primary.sessionType, primary.workspace, chatsObs, this.instantiationService, this.commandService);
