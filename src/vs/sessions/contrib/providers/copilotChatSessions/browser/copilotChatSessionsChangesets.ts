...
import { gitHubInfoEqual, IChat, IGitHubInfo, ISessionChangeset, ISessionChangesetOperation, ISessionChangesetOperationTarget, ISessionFileChange, ISessionWorkspace, SessionChangesetOperationScope, SessionChangesetOperationStatus, sessionFileChangesEqual } from '../../../../services/sessions/common/session.js';
...
export function createChangesets(
	...
	const isCloud = sessionType === AgentSessionProviders.Cloud;

	if (isCloud) {
		// Cloud sessions: show branch/all/last-turn changesets (review-oriented)
		return constObservable<ISessionChangeset[]>([
			new BranchChangesChangeset(workspaceObs, chatsObs, true),
			new AllChangesChangeset(chatsObs, changesetResolver),
			new LastTurnChangesChangeset(chatsObs, changesetResolver),
		]);
	}

	// Local sessions: only show Uncommitted Changes — no version selector dropdown
	return constObservable<ISessionChangeset[]>([
		new UncommittedChangesChangeset(workspaceObs, chatsObs, changesetResolver, commandService),
	]);
}
...
export class BranchChangesChangeset extends AbstractChangeset {
	...
	constructor(
		...
		this.isDefault = derived(reader => isDefaultValue && !isArchivedObs.read(reader));
...
export class UncommittedChangesChangeset extends AbstractChangeset {
	...
	private readonly _hasUncommittedObs: IObservable<number>;
	...
	constructor(
		...
		this.isDefault = this.isEnabled;

		this._hasUncommittedObs = derived(reader => {
		...
		const changesPromiseObs = derived(reader => {
			...
			this._hasUncommittedObs.read(reader);
