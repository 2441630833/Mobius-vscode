...
class ChangesMenuWorkbenchButtonBarWidget extends Disposable {
	...
	private _getButtonConfiguration(action: IAction, outgoingChanges: number, hasGitOperationInProgress: boolean, runningLabelObs: IObservable<string | IMarkdownString | undefined>): { showIcon: boolean; showLabel: boolean; isSecondary?: boolean; customLabel?: string | IMarkdownString; customLabelObs?: IObservable<string | IMarkdownString | undefined>; customClass?: string } | undefined {
		...
		if (
			...
			action.id === 'agentSession.gitCommitAndDone' ||
		...
		if (this.actionsContainer) {
			...
			this.renderDisposables.add(autorun(reader => {
				...
				const isAgentHostSession = isAgentHostSessionObs.read(reader);
					...
					// Use the operations-based button bar whenever the active changeset
					// advertises Changeset-scoped operations (e.g. local "Uncommitted
					// Changes" with a Commit button). Otherwise fall back to the menu-based
					// bar for plain git sessions (branch sync / PR actions).
					const operations = this.viewModel.activeSessionChangesetOperationsObs.read(reader);
					const hasChangesetOperations = operations.some(op => op.scopes.includes(SessionChangesetOperationScope.Changeset));

					const widget = (isAgentHostSession || hasChangesetOperations)
		...
		this.renderDisposables.add(autorun(reader => {
			if (this.viewModel.activeSessionIsLoadingObs.read(reader)) {
				return;
						...
						// Show the actions toolbar unless this is an untitled session with no
			// changeset operations (e.g. a fresh chat with nothing to commit).
			const activeSessionStatus = activeSessionStatusObs.read(reader);
			const isUntitled = activeSessionStatus === SessionStatus.Untitled;
			const operations = this.viewModel.activeSessionChangesetOperationsObs.read(reader);
			const hasChangesetOperations = operations.some(op => op.scopes.includes(SessionChangesetOperationScope.Changeset));
			if (this.actionsContainer) {
				dom.setVisibility(!isUntitled || hasChangesetOperations, this.actionsContainer);
			}
			...
			const hasEntries = stats !== undefined && stats.files > 0;
						...
						// Show the files header whenever there are entries or git repo or changeset ops.
			dom.setVisibility(hasGitRepository || hasEntries || hasChangesetOperations, this.filesHeaderNode!);
