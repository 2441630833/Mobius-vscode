...
suite('SessionsTerminalContribution', () => {
	...
	// --- Visible Mobius / agent tool terminals (hideFromUser: false) ---

	test('tracks visible Mobius tool terminals for the active session on create', async () => {
		const cwd = URI.file('/worktree');
		activeSessionObs.set(makeAgentSession({ sessionId: 'test:session-1', worktree: cwd, providerType: AgentSessionProviders.Background }), undefined);
		await tick();

		const mobiusTerminal = makeTerminalInstance(nextInstanceId++, cwd.fsPath);
		mobiusTerminal._testSetShellLaunchConfig({ hideFromUser: false } as ITerminalInstance['shellLaunchConfig']);
		terminalInstances.set(mobiusTerminal.instanceId, mobiusTerminal);
		onDidCreateInstance.fire(mobiusTerminal);
		await tick();

		// Switch away so session-1 terminals are no longer protected as active
		activeSessionObs.set(makeAgentSession({ sessionId: 'test:session-2', worktree: URI.file('/other'), providerType: AgentSessionProviders.Background }), undefined);
		await tick();
		moveToBackgroundCalls.length = 0;
		...
		onDidChangeSessions.fire({
			added: [],
			removed: [makeAgentSession({ sessionId: 'test:session-1', worktree: cwd, providerType: AgentSessionProviders.Background })],
			changed: [],
		});
		await tick();

		assert.ok(
			disposedInstances.some(i => i.instanceId === mobiusTerminal.instanceId),
			'visible Mobius tool terminal tracked to the session should be disposed when the session is removed'
		);
	});

	test('backgrounds visible Mobius tool terminals when switching away from their session', async () => {
		const cwd1 = URI.file('/cwd1');
		const cwd2 = URI.file('/cwd2');
		...
		const mobiusTerminal = makeTerminalInstance(nextInstanceId++, cwd1.fsPath);
		mobiusTerminal._testSetShellLaunchConfig({ hideFromUser: false } as ITerminalInstance['shellLaunchConfig']);
		terminalInstances.set(mobiusTerminal.instanceId, mobiusTerminal);
		onDidCreateInstance.fire(mobiusTerminal);
		await tick();
		...
		assert.ok(moveToBackgroundCalls.includes(mobiusTerminal.instanceId), 'visible Mobius tool terminal should be backgrounded with its session');
	});
