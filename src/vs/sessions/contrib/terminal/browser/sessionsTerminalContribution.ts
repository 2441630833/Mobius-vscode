...
export class SessionsTerminalContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		// Also associate newly created visible terminals (user + Mobius agent
		// tool shells) with the active session so the tab list stays session-scoped.
		this._register(this._terminalService.onDidCreateInstance(instance => {
			// Skip truly hidden tool terminals — managed by the chat tool lifecycle
			...
			if (instance.shellLaunchConfig.attachPersistentProcess && this._activeKey) {
				instance.getInitialCwd().then(cwd => {
					const availableInstance = this._getAvailableTerminal(instance, `handle restored terminal for ${cwd}`);
					...
					if (this._activeSessionId) {
						this._trackTerminalsForSession(this._activeSessionId, [availableInstance]);
						this._logService.trace(`[SessionsTerminal] Tracked restored terminal ${availableInstance.instanceId} for active session ${this._activeSessionId}`);
					}
				});
				return;
			}
			if (this._activeSessionId) {
				...
				this._logService.trace(`[SessionsTerminal] Tracked terminal ${instance.instanceId} for active session ${this._activeSessionId}`);
			}
		}));
