...
/** Mobius uses Continue; skip GitHub Copilot extension build/shim unless explicitly enabled. */
const skipCopilotBuild = process.env['SKIP_COPILOT_BUILD'] !== '0';
...
async function isWindowsPeFile(filePath: string): Promise<boolean> {
...
function runSignTool(args: string[]): Promise<{ exitCode: number | null; spawnError?: NodeJS.ErrnoException }> {
	return new Promise(resolve => {
		const proc = cp.spawn('signtool.exe', args);
		proc.on('error', spawnError => resolve({ exitCode: null, spawnError }));
		proc.on('exit', exitCode => resolve({ exitCode }));
	});
}

function hasAuthenticodeSignature(filePath: string): Promise<boolean> {
	return runSignTool(['verify', '/pa', filePath]).then(({ exitCode, spawnError }) => {
		if (spawnError) {
...
async function stripAuthenticodeSignature(filePath: string): Promise<void> {
	...
	const { exitCode, spawnError } = await runSignTool(['remove', '/s', filePath]);
	if (spawnError) {
		if (spawnError.code === 'ENOENT') {
			return;
		}
	...
	if (exitCode !== 0) {
		throw new Error(`signtool remove /s failed for ${filePath} (exit ${exitCode})`);
	}
}
...
function patchWin32DependenciesTask(destinationFolderName: string) {
	...
	return async () => {
		...
		])).flatMap(o => o).filter(dep => !/prebuilds[/\\](darwin|linux|linuxmusl)-/.test(dep));
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			if (!await isWindowsPeFile(fullPath)) {
				return;
			}
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		const packageTasks: task.Task[] = [
			...
			...(skipCopilotBuild ? [] : [prepareCopilotRipgrepShimTask(platform, arch, destinationFolderName)])
		];
		...
		if (useEsbuildTranspile) {
			...
			// Register so Mobius incremental packaging can invoke rebundle without a full vscode-*-min.
			task.task(esbuildBundleTask);
			vscodeTask = task.define(`vscode${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(
				copyCodiconsTask,
				cleanExtensionsBuildTask,
				compileNonNativeExtensionsBuildTask,
				...(skipCopilotBuild ? [] : [compileCopilotExtensionBuildTask]),
				compileExtensionMediaBuildTask,
				writeISODate('out-build'),
				esbuildBundleTask,
				vscodeTaskCI
