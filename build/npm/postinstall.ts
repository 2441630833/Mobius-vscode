...
function setNpmrcConfig(dir: string, env: NodeJS.ProcessEnv) {
	...
	// Use our bundled node-gyp version (Windows: Mobius wrapper disables Spectre mitigation for VS 2022)
	if (process.platform === 'win32') {
		env['MOBIUS_NODE_GYP_BIN'] = path.join(import.meta.dirname, 'gyp', 'node_modules', 'node-gyp', 'bin', 'node-gyp.js');
		env['npm_config_node_gyp'] = path.join(import.meta.dirname, '..', '..', '..', 'scripts', 'node-gyp-win.js');
	} else {
		env['npm_config_node_gyp'] = path.join(import.meta.dirname, 'gyp', 'node_modules', '.bin', 'node-gyp');
	}
