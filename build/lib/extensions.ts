...
/** npm ls exits non-zero for monorepo file: deps (e.g. Continue); stdout is still valid. */
function patchVsceNpmForDependencyTreeWarnings(): void {
	const npmModule = require(path.join(root, 'build/node_modules/@vscode/vsce/out/npm')) as {
		getDependencies: (cwd: string, dependencies: string | undefined, packagedDependencies: string[] | undefined) => Promise<string[]>;
		__mobiusPatched?: boolean;
	};
	if (npmModule.__mobiusPatched) {
		return;
	}
	const original = npmModule.getDependencies.bind(npmModule);
	npmModule.getDependencies = async (cwd, dependencies, packagedDependencies) => {
		if (dependencies === 'none' || dependencies === 'yarn') {
			return original(cwd, dependencies, packagedDependencies);
		}
		try {
			return await original(cwd, dependencies, packagedDependencies);
		} catch (err: unknown) {
			...
			if (!/ELSPROBLEMS/.test(combined)) {
				throw err;
			}
			try {
				const stdout = cp.execSync('npm list --production --parseable --depth=99999 --loglevel=error', {
					cwd,
					maxBuffer: 5000 * 1024,
					encoding: 'utf8',
				...
				const ex = execErr as { message?: string; stdout?: string | Buffer; stderr?: string | Buffer };
				const stdout = ex.stdout?.toString() ?? '';
				if (/ELSPROBLEMS/.test([ex.message, stdout, ex.stderr?.toString()].join('\n')) && stdout) {
					return stdout.split(/[\r\n]/).filter(dir => path.isAbsolute(dir));
				}
				throw execErr;
			}
	...
	npmModule.__mobiusPatched = true;
}
...
function fromLocalNormal(extensionPath: string): Stream {
	...
	// Continue is prepackaged (out/extension.js + out/node_modules); npm ls pulls the whole monorepo.
	...
	if (!isContinue) {
		patchVsceNpmForDependencyTreeWarnings();
	}
...
export function packageCopilotExtensionStream(disableMangle: boolean): Stream {
	...
	const streams: Stream[] = [localExtensionsStream];
	...
	return es.merge(...streams).pipe(util2.setExecutableBit(['**/*.sh']));
}
