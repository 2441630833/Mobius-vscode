...
function resolveCommit(): string {
	const fromEnvOrGit = getVersion(repoPath);
	if (fromEnvOrGit && /^[0-9a-f]{40}$/i.test(fromEnvOrGit.trim())) {
		return fromEnvOrGit.trim();
	}

	try {
		const sha = cp.execFileSync('git', ['rev-parse', 'HEAD'], { cwd: repoPath, encoding: 'utf8' }).trim();
